...
// Assumption: Horizontal-scroll on mobile, 3-4 column grid on desktop
// Features: Dark background, scroll-snap on mobile, typed props for projects
...
// Generate example projects using real data from SiteConfig
const generateExampleProjects = (): Project[] => {
  const { serviceAreas, services } = SiteConfig;
  
  // Get a subset of services and areas for variety
  const exampleServices = services.slice(0, 8);
  const exampleAreas = serviceAreas.slice(0, 6);
  
  // Create realistic project examples
  const projectTemplates = [
    ...
    {
      title: "Complete Diagnostic & Repair",
    ...
    },
    {
      ...
      service: "Car Transmission Repair",
      area: "Dubailand",
    },
    {
      title: "Full Engine Diagnostics",
      service: "Car Service Dubai",
      area: "Al Barsha",
    },
  ];
  ...
  return projectTemplates.map((template, index) => ({
    ...
    image: `/images/projects/project-${(index % 3) + 1}.jpg`,
  }));
};


export function Projects({ projects = generateExampleProjects() }: ProjectsProps) {
